...
describe('list page works correctly', () => {
  ...
  it('should have 5 starting elements', () => {
    ...
    cy.get(getCircle(0)).invoke('text').should((text) => expect(text).not.to.eq(''));
    cy.get(getCircle(1)).invoke('text').should((text) => expect(text).not.to.eq(''));
    cy.get(getCircle(2)).invoke('text').should((text) => expect(text).not.to.eq(''));
    cy.get(getCircle(3)).invoke('text').should((text) => expect(text).not.to.eq(''));
    cy.get(getCircle(4)).invoke('text').should((text) => expect(text).not.to.eq(''));
  })
  ...
  it('can add an element to the stack head', () => {
    ...
    cy.get(getCircle(100)).should('contain', '1').should('have.css', 'border-color', CHANGING_COLOR);
    cy.get(getCircle(100)).should('not.exist');
    cy.debug(cy.get(getCircle(0)))
    cy.get(getCircle(0)).should('contain', '1').should('have.css', 'border-color', MODIFIED_COLOR)
  ...
  it('can add an element to the stack tail', () => {
    ...
    cy.get(getCircle(100)).should('contain', '1').should('have.css', 'border-color', CHANGING_COLOR);
    cy.get(getCircle(100)).should('not.exist');
    cy.get(getCircle(5)).should('contain', '1').should('have.css', 'border-color', MODIFIED_COLOR);
    cy.tick(SHORT_DELAY_IN_MS);
    cy.get(getCircle(5)).should('have.css', 'border-color', DEFAULT_COLOR).next().next().should('have.text', 'tail');
    cy.get('@inputNumber').should('have.value', '');
  })
  ...
  it('remove add an element from the stack head', () => {
    ...
    let text1 = '';
    let text2 = '';
    cy.get(getCircle(0)).should(($text) => text1 = $text.text());
    cy.get(getCircle(1)).should(($text) => text2 = $text.text());
    cy.get('@removeHead').click();
    cy.tick(SHORT_DELAY_IN_MS);
    cy.get(getCircle(100)).should('have.css', 'border-color', CHANGING_COLOR)
        .invoke('text').should((text) => expect(text).to.eq(text1));
    cy.get(getCircle(100)).should('not.exist');
    cy.get(getCircle(0)).should('contain', text2).should('have.css', 'border-color', DEFAULT_COLOR)
      .prev().should('have.text', 'head');
    cy.tick(SHORT_DELAY_IN_MS);
  })
  ...
  it('remove add an element from the stack tail', () => {
    cy.clock();
    let text1 = '';
    cy.get(getCircle(4)).should(($text) => text1 = $text.text());
  ...
  it('can add an element to the stack by index', () => {
    ...
    cy.tick(SHORT_DELAY_IN_MS);
    cy.get(getCircle(100)).should('contain', '1').should('have.css', 'border-color', CHANGING_COLOR);
    cy.get(getCircle(0)).should('have.css', 'border-color', CHANGING_COLOR)
    cy.get(getCircle(0)).prev().invoke('text').should((text) => expect(text).to.eq('1'));
    cy.get(getCircle(0)).prev().invoke('text').should((text) => expect(text).not.to.eq('1'));
    cy.get(getCircle(1)).prev().invoke('text').should((text) => expect(text).to.eq('1'));
    cy.tick(SHORT_DELAY_IN_MS);
    cy.get(getCircle(1)).should('contain', '1').should('have.css', 'border-color', MODIFIED_COLOR);
    cy.get(getCircle(2)).should('contain', text2).should('have.css', 'border-color', CHANGING_COLOR);
    cy.get(getCircle(100)).should('not.exist');
    cy.tick(SHORT_DELAY_IN_MS);
    cy.get(getCircle(0)).should('have.css', 'border-color', DEFAULT_COLOR);
    cy.get(getCircle(1)).should('have.css', 'border-color', DEFAULT_COLOR);
    cy.get(getCircle(2)).should('have.css', 'border-color', DEFAULT_COLOR);
    cy.get('@inputNumber').should('have.value', '');
  })
